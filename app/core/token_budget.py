...
async def record_token_usage(
    ...
    await db.execute(
        ...
        {"tid": tenant_id, "tokens": actual_tokens, "default_quota": settings.token_quota_starter},
    )
    await db.commit()
    logger.info(
        "token_budget.recorded tenant=%s tokens=%d", tenant_id, actual_tokens
    )
...
async def get_all_usage(db: AsyncSession) -> list[dict]:
    ...
    result = await db.execute(
        text(
            "SELECT tenant_id, period_month, tokens_used, token_quota "
    ...
    return [
        {
            ...
            "estimated_cost_usd": round(row.tokens_used / 1000 * 0.00015, 4),
